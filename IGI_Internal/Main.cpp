...
DWORD WINAPI MainThread(LPVOID lpThreadParameter) {
	...
	LOG_CONSOLE("[+]Dll Attached");
	LOG_CONSOLE("Initializing Pointers...");
	InitPointers();
	LOG_CONSOLE("Initializing Pointers DONE...");
	...
	while (!GT_IsKeyPressed(VK_END)) {
		...
		//}
			...
			*(INT*)(*((PINT)0x00567C8C) + 0x28) = 4;
			...
			LevelLoadRestart();
			LOG_CONSOLE("LevelLoadRestart done");
			...
			//	LOG_CONSOLE("WaitingForSingleObject.....");
...
void __cdecl LoadLevelMenuDetour() {
	...
	LOG_CONSOLE("LoadLevelMenuDetour ");
	...
	SetFramesVar(0x1e);
	*(INT*)(*((PINT)0x00567C8C) + 0x28) = 4;
	LOG_CONSOLE("LoadLevelMenuDetour Done");
	//LoadLevelMenuOut();
}
...
void LevelLoadRestart() {
	*(PINT)0x00936274 = 0;//Disable Warnings.
	HWND hwnd = GT_FindGameWindow("IGI");
	LOG_CONSOLE("LevelLoadRestart");
	LPVOID aLevelPathAddr = (LPVOID)0x0057B568;

	//HashInit('1');
	//LOG_CONSOLE("LevelLoadRestart : HashInit");
	UpdateQTask();
	LOG_CONSOLE("LevelLoadRestart : UpdateQTask");
	LevelLoad((int)aLevelPathAddr, 0x1E, *(PINT)0x04A27C18, 0x1);
	LOG_CONSOLE("LevelLoadRestart : LevelLoad");
	//HashReset();
	//LOG_CONSOLE("LevelLoadRestart : HashReset");
	SetForegroundWindow(hwnd);
}
...
MH_STATUS DLL_FreezeThread(LPVOID pTarget, BOOL enable) {
	...
	if (g_hHeap != NULL)
	{
		...
		else
		{
			...
			LOG_CONSOLE("FindHookEntry pos : %p", pos);

			if (pos != INVALID_HOOK_POS)
			{
				LOG_CONSOLE("FindHookEntry Valid pos : ");
				if (g_hooks.pItems[pos].isEnabled != enable)
				{
					...
					if (mh_status == MH_OK)
					{
						LOG_CONSOLE("Freeze thread at pos : %p", pos);
						LevelLoadRestart();
						Unfreeze(&threads);
						LOG_CONSOLE("Freeze MH_OK ");
					}
				}
				else
				{
...
void LevelStartPatch() {
	*(PINT)0x00936274 = 0;//Disable Warnings.
	int gameLevel = *((PDWORD)0x539560);
	HWND hwnd = GT_FindGameWindow("IGI");
	LOG_CONSOLE("LevelStartPatch Run");
...
bool GetThreadState(DWORD tid) {
	...
	if (u32_Error)
	{
		LOG_CONSOLE("Error 0x%X capturing processes.%p", std::to_string(u32_Error));
		return false;
	}
	...
	if (!pk_Proc)
	{
		LOG_CONSOLE("The process does not exist.\n");
		return false;
	}
	...
	if (!pk_Thread)
	{
		LOG_CONSOLE("The thread does not exist.\n");
		return false;
	}

	BOOL b_Suspend;
	i_Proc.IsThreadSuspended(pk_Thread, &b_Suspend);
	LOG_CONSOLE("GetThreadState Suspended : %d,", b_Suspend);
	return b_Suspend;
}
...
void StartThreadTimer(HANDLE h_thread, DWORD seconds, DWORD tid) {
	...
	LOG_CONSOLE("StartThreadTimer called");
	Timer tHello;
	tHello.start(std::chrono::seconds(seconds), [&] {
		if (thread_state) {
			ResumeThread(h_thread);
			CloseHandle(h_thread);
			LOG_CONSOLE("WaitForSingleObject done");
